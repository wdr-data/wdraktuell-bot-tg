...
export const settingsMiddleware = async (ctx, next) => {
    ...
    ctx.track = async (category, action, label, value) => {};
    ...
    if (trackingItem) {
        ...
        if (ctx.trackingEnabled) {
            ...
            ctx.track = (category, action, label, value) =>
                ctx.webtrekk.track(category, action, label, value);
        }
    } else {
        ctx.trackingEnabled = undefined;
    }
...
export const analyticsMiddleware = async (ctx, next) => {
    ...
    if (ctx.data && ctx.data.tracking) {
        ctx.track(
            ...
            ctx.data.tracking.action,
            ctx.data.tracking.label,
            ctx.data.tracking.value,
        );
    }
    return next();
};
