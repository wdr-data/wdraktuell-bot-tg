...
export default async (
    ...
    if (ctx.data.type === 'push') {
        ...
        const report = push.reports.find((r) => r.id === ctx.data.report);
        ...
        if (!isEndOfReport) {
            ...
            if (ctx.data.link) {
                buttonLink = Markup.urlButton(`🔗 ${report.short_headline}`, ctx.data.link);
            }
        }
