...
export async function uploadAttachment(url) {
    ...
    const responseMapping = () => {
        switch (type) {
        case 'image': return response.photo.slice(-1)[0];
        case 'document': return response.document;
        case 'audio': return response.audio;
        case 'video': return response.video;
        }
    };

    return responseMapping().file_id;
}
